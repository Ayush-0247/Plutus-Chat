...
import { playMessageSentSound } from '../services/soundEffects.js';
 ...
 <div className="w-full h-[670px] box-border px-1 sm:px-2 py-1 sm:py-3 flex flex-col font-mono bg-slate-50">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <div className="flex items-center gap-1.5 bg-slate-50 px-2.5 py-1.5 border-2 border-slate-900">
              <span className="text-slate-500 text-[10px] uppercase font-black">SESSION:</span>
              <span className="font-black text-indigo-700 tracking-wider">{sessionData.sessionId}</span>
              <button
            ...
            <div className="flex items-center gap-1.5 bg-slate-50 px-2.5 py-1.5 border-2 border-slate-900">
              <span className="text-slate-500 text-[10px] uppercase font-black">PASSKEY:</span>
              <span className="font-black text-amber-700 tracking-wider">{sessionData.passkey}</span>
              <button
                onClick={handleCopyKey}
                title="Copy Passkey"
                className="p-1 hover:text-amber-700 text-slate-500 transition-colors"
              >
                {copiedKey ? <Check className="w-3.5 h-3.5 text-amber-600" /> : <Copy className="w-3.5 h-3.5" />}
              </button>
            </div>
            ...
            >
              {copiedLink ? <Check className="w-3.5 h-3.5 text-indigo-700" /> : <Copy className="w-3.5 h-3.5" />}
              <span>{copiedLink ? 'INVITE LINK COPIED!' : 'COPY INVITE LINK'}</span>
            </button>

            <button
              id="open_test_tab_button"
              onClick={handleOpenTestTab}
              title="Open a new browser tab with this session pre-loaded to test multi-user chat"
              className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-slate-100 border-2 border-slate-900 text-slate-900 font-bold text-xs transition-colors shadow-[2px_2px_0px_0px_rgba(15,23,42,1)] active:translate-x-[1px] active:translate-y-[1px]"
            >
          ...
          {/* Right Action: End Session (Owner) or Leave Session (Joiner) */}
          <div className="flex items-center gap-2">
            {/* Mobile Participants toggle */}
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="md:hidden flex items-center gap-1.5 px-2.5 py-1.5 bg-white border-2 border-slate-900 text-slate-900 text-xs font-bold"
            >
            ...
            {isOwner ? (
              <button
                id="owner_end_session_button"
                onClick={() => setShowEndConfirm(true)}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-600 border-2 border-slate-900 hover:bg-rose-700 text-white text-xs font-black uppercase tracking-wider transition-colors shadow-[3px_3px_0px_0px_rgba(15,23,42,1)] active:translate-x-[1px] active:translate-y-[1px] focus:outline-none"
              >
              ...
              <button
                id="joiner_leave_session_button"
                onClick={onLeaveSession}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-slate-100 border-2 border-slate-900 text-slate-900 text-xs font-bold uppercase tracking-wider transition-colors shadow-[2px_2px_0px_0px_rgba(15,23,42,1)] active:translate-x-[1px] active:translate-y-[1px]"
              >
      ...
      <div className="flex-1 flex gap-3 min-h-0 relative">
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            {sessionData.participants.map((p) => {
              const isCurrentUser = p.participantId === sessionData.participantId;
              return (
                <div
                  key={p.participantId}
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    <span
                      className={`w-2 h-2 rounded-full shrink-0 ${
                        p.isOwner ? 'bg-amber-500 ring-2 ring-amber-400/50' : 'bg-emerald-600'
                      }`}
                    />
          ...
          <div className="pt-3 mt-3 border-t-2 border-slate-200 text-[10px] text-slate-600 font-bold space-y-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-emerald-700 font-black">RAM MAP()</span>
            </div>
            <div className="flex items-center justify-between">
              <span>PERSISTENCE:</span>
              <span className="text-slate-500">0 BYTES (NO DB)</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col bg-white border-2 border-slate-900 rounded-none overflow-hidden shadow-[4px_4px_0px_0px_rgba(15,23,42,1)]">
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-slate-50/60">
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400 text-xs">
                ...
                <p className="font-black text-slate-700 uppercase">Secure Line Connected</p>
                <p className="max-w-xs mt-1 text-slate-500">
                  Send a message to begin. Real-time text is broadcast directly to active participant RAM.
                </p>
              </div>
            ) : (
              messages.map((m) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-1.5 text-[10px] text-slate-500 mb-1 px-1 font-bold">
                      <span className="font-black text-slate-900">{isMe ? 'You' : m.senderName}</span>
                      {m.isOwner && (
          ...
          {/* Message Input Bar */}
          <form onSubmit={handleSend} className="p-3 border-t-2 border-slate-900 bg-white">
            <div className="flex items-center gap-2">
              <input
                ref={inputRef}
                type="text"
                ...
                placeholder="Type real-time ephemeral message... (Enter to send)"
                maxLength={2000}
                autoFocus
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
          ...
          >
            ...
            <p className="text-xs text-slate-600 leading-relaxed mb-5">
              This will initiate a 15-second server-authoritative countdown (5-second warning + 10-second timer) and disconnect all {sessionData.participants.length} participants. All in-memory state will be permanently wiped.
            </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
          ...
          >
            ...
            <p className="text-xs text-slate-600 leading-relaxed mb-2">
              Are you sure you want to remove <strong className="text-slate-900 font-bold">{kickTarget.username}</strong>?
            </p>
            <p className="text-[11px] text-amber-950 font-bold leading-relaxed mb-5 p-2.5 bg-amber-50 border-2 border-amber-500">
              PRD Invariant 3: Kicked participants are permanently banned from rejoining this active session even if they have the link and passkey.
            </p>
            <div className="flex items-center justify-end gap-3">
              <button
