// Web Audio API based synthesized sound effects for Ephemeral Secure Line
...
export function playCountdownTickSound(isUrgent = false) {
  if (!soundEnabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
    ...
    gain.gain.setValueAtTime(isUrgent ? 0.12 : 0.06, now);
...
export function playPurgedSound() {
  if (!soundEnabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
...
export function playIncomingCallSound() {
  ...
  try {
    ...
    const now = ctx.currentTime;
    // Ringtone pattern: two dual-tone bursts
...
export const playCallIncomingSound = playIncomingCallSound;
...
export function playFileCompleteSound() {
  if (!soundEnabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
